...
/**
 * Game controller conducts basic game logic for a chess game and certain
...
public class GameController {
    ...
    public void move(BoardIF board, Position from, Position to)
            throws GameOverCheckMateException, GameOverStaleMateException {
        ...
        boolean whiteTurn = (board.getSquare(from.getRank(), from.getFile()).getPiece().getColor() == GameColor.WHITE
                && playerTurn);
        boolean blackTurn = (board.getSquare(from.getRank(), from.getFile()).getPiece().getColor() == GameColor.BLACK
                && !playerTurn);
     ...
     */
    private void endGameHelp(BoardIF board, GameColor color)
            throws GameOverStaleMateException, GameOverCheckMateException {
    ...
    /**
     * prints the players turn and flips the whose turn it is for the undo
     * method.
     *
     ...
     * @return a Position object representing the requested position from the
     *         user or null if the requested position is invalid
     */
    public Position getPosition(String pos, SquareIF[][] squares) {
        ...
        try {
            ...
            if (file != null && rank != null) {
                ...
                // otherwise it is not a valid file and/or rank and we return
                // null
            } else {
                refinedPos = null;
            }
